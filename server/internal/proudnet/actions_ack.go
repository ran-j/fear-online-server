...
var staticResponses = map[uint16]response{
	...
	// CloseSuccess requires a destination: 4 = lobby, 5 = room.
	// ZNetwork callbacks: 0x100679A0 / 0x1007B140 / 0x10078330.
	0x9859: {answer: 0x98BD},                 // ItemC2S::RequestOpen
	0x985A: {answer: 0x98BF, destination: 4}, // ItemC2S::RequestClose
	0x99E9: {answer: 0x9A4D},                 // RecipeC2S::RequestOpen
	0x99EA: {answer: 0x9A4F},                 // RecipeC2S::RequestOpenNew
	0x99EB: {answer: 0x9A51, destination: 4}, // RecipeC2S::RequestClose
	0x9AB1: {answer: 0x9B15},                 // PerkC2S::RequestOpen
	0x9AB2: {answer: 0x9B17},                 // PerkC2S::RequestOpenNew
	0x9AB3: {answer: 0x9B19, destination: 4}, // PerkC2S::RequestClose
}
...
func NewGameActionHandle(server *Server) {
	for request, resp := range staticResponses {
		...
		server.Handle(request, func(session *Session, _ Message) error {
			body := resp.body
			if resp.destination != 0 {
				destination := resp.destination
				if sessionRoom(session) != nil {
					destination = 5
				}
				body = []byte{destination}
			}
			return session.SendPlain(Message{ID: resp.answer, Body: body})
		})
	}
}
